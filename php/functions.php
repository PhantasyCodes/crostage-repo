...
function editUser($connection, $first_name, $last_name, $username, $number, $email, $password, $filename, $location, $sesh_id) {
    ...
    $sql = "UPDATE users SET first_name = 'davi', last_name = case when '$last_name' is null then last_name else '$last_name', username = case when '$username' is null then username else '$username', number = case when '$number' is null then number else '$number', email = case when '$email' is null then email else '$email', password = case when '$password' is null then password else '$password', profile_picture =  case when '$filename' is null then profile_picture else '$filename' WHERE user_id = 3";
